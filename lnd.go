...
func Main(cfg *Config, lisCfg ListenerCfg, shutdownChan <-chan struct{}) error {
	...
	var network string
	switch {
	case cfg.Bitcoin.TestNet3 || cfg.Earthcoin.TestNet3:
		network = "testnet"

	case cfg.Bitcoin.MainNet || cfg.Earthcoin.MainNet:
		network = "mainnet"

	case cfg.Bitcoin.SimNet || cfg.Earthcoin.SimNet:
		network = "simnet"

	case cfg.Bitcoin.RegTest || cfg.Earthcoin.RegTest:
		network = "regtest"
	}
	...
	if !(cfg.Bitcoin.RegTest || cfg.Bitcoin.SimNet ||
		cfg.Earthcoin.RegTest || cfg.Earthcoin.SimNet) {
...
// This function is taken from https://github.com/eacsuite/eacd
