...
import (
	...
	"github.com/eacsuite/lnd/watchtower/blob"
)
...
// policy's fee rate.
func (p *Policy) ComputeAltruistOutput(totalAmt eacutil.Amount,
	txWeight int64) (eacutil.Amount, error) {
...
func (p *Policy) ComputeRewardOutputs(totalAmt eacutil.Amount,
	txWeight int64) (eacutil.Amount, eacutil.Amount, error) {
...
func ComputeRewardAmount(total eacutil.Amount, base, rate uint32) eacutil.Amount {
	rewardBase := eacutil.Amount(base)
	rewardRate := eacutil.Amount(rate)
...
func (p *Policy) ComputeJusticeTxOuts(totalAmt eacutil.Amount, txWeight int64,
