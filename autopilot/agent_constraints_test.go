...
func TestConstraintsChannelBudget(t *testing.T) {
	...
	const (
		minChanSize = 0
		maxChanSize = eacutil.Amount(eacutil.SatoshiPerBitcoin)
	...
	testCases := []struct {
		channels  []Channel
		walletAmt eacutil.Amount

		needMore     bool
		amtAvailable eacutil.Amount
		numMore      uint32
	}{
		...
		// channels.
		{
			[]Channel{
				{
					ChanID:   randChanID(),
					Capacity: eacutil.Amount(prand.Int31()),
				},
				{
					ChanID:   randChanID(),
					Capacity: eacutil.Amount(prand.Int31()),
			...
			eacutil.Amount(eacutil.SatoshiPerBitcoin * 10),
			false,
			0,
			0,
		},
		...
		// threshold.
		{
			[]Channel{
				{
					ChanID:   randChanID(),
					Capacity: eacutil.Amount(eacutil.SatoshiPerBitcoin),
				},
				{
					ChanID:   randChanID(),
					Capacity: eacutil.Amount(eacutil.SatoshiPerBitcoin),
			...
			eacutil.Amount(eacutil.SatoshiPerBitcoin * 2),
			false,
			0,
			0,
		},
		...
		// limit is 3.
		{
			...
			eacutil.Amount(eacutil.SatoshiPerBitcoin * 9),
			true,
			eacutil.Amount(eacutil.SatoshiPerBitcoin * 4),
			2,
		},
		...
		// channel as the limit is 3.
		{
			[]Channel{
				...
				{
					ChanID:   randChanID(),
					Capacity: eacutil.Amount(eacutil.SatoshiPerBitcoin * 3),
				},
			},
			eacutil.Amount(eacutil.SatoshiPerBitcoin * 10),
			true,
			eacutil.Amount(eacutil.SatoshiPerBitcoin * 3),
			1,
		},
		...
		// threshold.
		{
			...
			eacutil.Amount(eacutil.SatoshiPerBitcoin),
			false,
			0,
			0,
		},
