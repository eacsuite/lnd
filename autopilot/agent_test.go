...
func (m *mockConstraints) ChannelBudget(chans []Channel,
	balance eacutil.Amount) (eacutil.Amount, uint32) {
...
func (m *mockConstraints) MinChanSize() eacutil.Amount {
	return 1e7
}
func (m *mockConstraints) MaxChanSize() eacutil.Amount {
	return 1e8
}
...
func (m *mockHeuristic) NodeScores(g ChannelGraph, chans []Channel,
	chanSize eacutil.Amount, nodes map[NodeID]struct{}) (
...
func (m *mockChanController) SpliceIn(chanPoint *wire.OutPoint,
	amt eacutil.Amount) (*Channel, error) {
	return nil, nil
}
...
func setup(t *testing.T, initialChans []Channel) (*testContext, func()) {
	...
	var availableFunds eacutil.Amount = 10 * eacutil.SatoshiPerBitcoin
	...
	testCfg := Config{
		...
		Heuristic:      heuristic,
		ChanController: chanController,
		WalletBalance: func() (eacutil.Amount, error) {
			ctx.Lock()
			defer ctx.Unlock()
...
func TestAgentChannelOpenSignal(t *testing.T) {
	...
	newChan := Channel{
		ChanID:   randChanID(),
		Capacity: eacutil.SatoshiPerBitcoin,
	}
...
func TestAgentHeuristicUpdateSignal(t *testing.T) {
	...
	respondMoreChans(t, testCtx,
		moreChansResp{
			numMore: 1,
			amt:     1 * eacutil.SatoshiPerBitcoin,
		},
	)
...
func TestAgentChannelFailureSignal(t *testing.T) {
	...
	respondMoreChans(t, testCtx, moreChansResp{1, 5 * eacutil.SatoshiPerBitcoin})
...
func TestAgentChannelCloseSignal(t *testing.T) {
	...
	initialChans := []Channel{
		...
		{
			ChanID:   randChanID(),
			Capacity: eacutil.SatoshiPerBitcoin * 2,
		},
	}
...
func TestAgentBalanceUpdate(t *testing.T) {
	...
	testCtx.Lock()
	testCtx.walletBalance += eacutil.SatoshiPerBitcoin * 5
	testCtx.Unlock()
...
func TestAgentImmediateAttach(t *testing.T) {
	...
	respondMoreChans(t, testCtx,
		moreChansResp{
			numMore: numChans,
			amt:     5 * eacutil.SatoshiPerBitcoin,
		},
	)
	...
	for i := 0; i < numChans; i++ {
		...
		case openChan := <-chanController.openChanSignals:
			if openChan.amt != eacutil.SatoshiPerBitcoin {
				t.Fatalf("invalid chan amt: expected %v, got %v",
					eacutil.SatoshiPerBitcoin, openChan.amt)
			}
...
func TestAgentPendingChannelState(t *testing.T) {
	...
	// heuristic.
	testCtx.Lock()
	testCtx.walletBalance += 0.4 * eacutil.SatoshiPerBitcoin
	testCtx.Unlock()
	...
	// channels.
	select {
	case testCtx.constraints.moreChansResps <- moreChansResp{1, eacutil.SatoshiPerBitcoin}:
	case <-time.After(time.Second * 10):
		t.Fatalf("need more chans wasn't queried in time")
	}
...
func respondWithScores(t *testing.T, testCtx *testContext,
	channelBudget eacutil.Amount, existingChans, newChans int,
...
func checkChannelOpens(t *testing.T, testCtx *testContext,
	allocation eacutil.Amount, numChans int) []NodeID {

	var nodes []NodeID

	// The agent should attempt to open channels, totaling what we expect.
	var totalAllocation eacutil.Amount
	chanController := testCtx.chanController.(*mockChanController)
	for i := 0; i < numChans; i++ {
		select {
		case openChan := <-chanController.openChanSignals:
...
func TestAgentChannelSizeAllocation(t *testing.T) {
	...
	var channelBudget eacutil.Amount = 5 * eacutil.SatoshiPerBitcoin
	...
	// channels.
	expectedAllocation := testCtx.constraints.MaxChanSize() * eacutil.Amount(numNewChannels)
	...
	channelBudget = eacutil.SatoshiPerBitcoin * 3 / 2
	...
	// our budget.
	channelBudget = eacutil.SatoshiPerBitcoin * 5
	numNewChannels = 2
	respondWithScores(
		t, testCtx, channelBudget, numExistingChannels,
		numNewChannels, nodeScores,
