...
func createTestChannelState(t *testing.T, cdb *DB) *OpenChannel {
	...
	localCfg := ChannelConfig{
		ChannelConstraints: ChannelConstraints{
			DustLimit:        eacutil.Amount(rand.Int63()),
	...
	return &OpenChannel{
		...
		LocalCommitment: ChannelCommitment{
			...
			CommitFee:     eacutil.Amount(rand.Int63()),
			FeePerKw:      eacutil.Amount(5000),
			CommitTx:      testTx,
...
func TestOpenChannelPutGetDelete(t *testing.T) {
	...
	closeSummary := &ChannelCloseSummary{
		...
		SettledBalance:    eacutil.Amount(500),
		TimeLockedBalance: eacutil.Amount(10000),
		IsPending:         false,
