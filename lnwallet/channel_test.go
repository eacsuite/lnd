...
func testAddSettleWorkflow(t *testing.T, tweakless bool) {
	...
	htlcAmt := lnwire.NewMSatFromSatoshis(eacutil.SatoshiPerBitcoin)
	...
	mSatTransferred := lnwire.NewMSatFromSatoshis(eacutil.SatoshiPerBitcoin)
...
func TestCheckCommitTxSize(t *testing.T) {
	...
	checkSize := func(channel *LightningChannel, count int) {
		...
		actualCost := blockchain.GetTransactionWeight(eacutil.NewTx(commitTx))
...
func TestHTLCDustLimit(t *testing.T) {
	...
	// Bob's dust limit.
	htlcSat := (eacutil.Amount(500) + HtlcTimeoutFee(
...
func TestHTLCSigNumber(t *testing.T) {
	...
	createChanWithHTLC := func(htlcValues ...eacutil.Amount) (
	...
	belowDust := eacutil.Amount(500) + HtlcTimeoutFee(
		channeldb.SingleFunderTweaklessBit, feePerKw,
	)
	aboveDust := eacutil.Amount(1400) + HtlcSuccessFee(
		channeldb.SingleFunderTweaklessBit, feePerKw,
	)
...
func TestCancelHTLC(t *testing.T) {
	...
	// of the new HTLC.
	aliceExpectedBalance := eacutil.Amount(eacutil.SatoshiPerBitcoin*4) -
	...
	expectedBalance := eacutil.Amount(eacutil.SatoshiPerBitcoin * 5)
...
func TestCooperativeCloseDustAdherence(t *testing.T) {
	...
	setDustLimit := func(dustVal eacutil.Amount) {
	...
	// satoshis.
	dustLimit := eacutil.Amount(10000)
	...
	aliceFee := eacutil.Amount(aliceChannel.CalcFee(aliceFeeRate)) + 1000
	aliceSig, _, _, err := aliceChannel.CreateCloseProposal(
		aliceFee, aliceDeliveryScript, bobDeliveryScript,
	)
	if err != nil {
	...
	bobFee := eacutil.Amount(bobChannel.CalcFee(bobFeeRate)) + 1000
	bobSig, _, _, err := bobChannel.CreateCloseProposal(
		bobFee, bobDeliveryScript, aliceDeliveryScript,
	)
	if err != nil {
	...
	aliceBal := lnwire.NewMSatFromSatoshis(eacutil.SatoshiPerBitcoin)
...
func TestUpdateFeeConcurrentSig(t *testing.T) {
	...
	htlc := &lnwire.UpdateAddHTLC{
		PaymentHash: paymentHash,
		Amount:      eacutil.SatoshiPerBitcoin,
		Expiry:      uint32(5),
	}
...
func TestAddHTLCNegativeBalance(t *testing.T) {
	...
	htlcAmt = lnwire.NewMSatFromSatoshis(2 * eacutil.SatoshiPerBitcoin)
...
func TestFeeUpdateOldDiskFormat(t *testing.T) {
	...
	htlcAmt := lnwire.NewMSatFromSatoshis(0.1 * eacutil.SatoshiPerBitcoin)
	const numHTLCs = 30
...
func TestChanAvailableBalanceNearHtlcFee(t *testing.T) {
	...
	aliceBalance := lnwire.NewMSatFromSatoshis(5 * eacutil.SatoshiPerBitcoin)
	bobBalance := lnwire.NewMSatFromSatoshis(5 * eacutil.SatoshiPerBitcoin)
...
func TestDesyncHTLCs(t *testing.T) {
	...
	htlcAmt := lnwire.NewMSatFromSatoshis(4.1 * eacutil.SatoshiPerBitcoin)
	htlc, _ := createHTLC(0, htlcAmt)
	...
	htlcAmt = lnwire.NewMSatFromSatoshis(1 * eacutil.SatoshiPerBitcoin)
...
func TestMaxPendingAmount(t *testing.T) {
	...
	maxPending := lnwire.NewMSatFromSatoshis(eacutil.SatoshiPerBitcoin * 3)
	...
	const numHTLCs = 2
	htlcAmt := lnwire.NewMSatFromSatoshis(1.5 * eacutil.SatoshiPerBitcoin)
	for i := 0; i < numHTLCs; i++ {
		htlc, _ := createHTLC(i, htlcAmt)
		if _, err := aliceChannel.AddHTLC(htlc, nil); err != nil {
			t.Fatalf("unable to add htlc: %v", err)
...
func assertChannelBalances(t *testing.T, alice, bob *LightningChannel,
	aliceBalance, bobBalance eacutil.Amount) {
...
func TestChanReserve(t *testing.T) {
	...
	setupChannels := func() (*LightningChannel, *LightningChannel, func()) {
		...
		aliceMinReserve := eacutil.Amount(0.5 *
			eacutil.SatoshiPerBitcoin)
		...
		bobMinReserve := eacutil.Amount(6 * eacutil.SatoshiPerBitcoin)
	...
	//	Alice:	4.5
	//	Bob:	5.0
	htlcAmt := lnwire.NewMSatFromSatoshis(0.5 * eacutil.SatoshiPerBitcoin)
	...
	assertChannelBalances(
		...
		eacutil.SatoshiPerBitcoin*4.5-commitFee, eacutil.SatoshiPerBitcoin*5,
	)
	...
	//	Alice:	1.5
	//	Bob:	9.5
	htlcAmt = lnwire.NewMSatFromSatoshis(3.5 * eacutil.SatoshiPerBitcoin)
	...
	assertChannelBalances(
		t, aliceChannel, bobChannel,
		eacutil.SatoshiPerBitcoin*3-commitFee, eacutil.SatoshiPerBitcoin*5,
	)
	...
	assertChannelBalances(
		t, aliceChannel, bobChannel,
		eacutil.SatoshiPerBitcoin*3-commitFee, eacutil.SatoshiPerBitcoin*7,
	)
	...
	assertChannelBalances(
		t, aliceChannel, bobChannel,
		eacutil.SatoshiPerBitcoin*3-commitFee, eacutil.SatoshiPerBitcoin*6,
	)
}
...
func TestChanReserveRemoteInitiator(t *testing.T) {
	...
	commitFee := aliceChannel.channelState.LocalCommitment.CommitFee
	aliceMinReserve := 5*eacutil.SatoshiPerBitcoin - commitFee
...
func TestChanReserveLocalInitiatorDustHtlc(t *testing.T) {
	...
	commitFee := aliceChannel.channelState.LocalCommitment.CommitFee
	aliceMinReserve := 5*eacutil.SatoshiPerBitcoin - commitFee - htlcSat
...
func TestMinHTLC(t *testing.T) {
	...
	minValue := lnwire.NewMSatFromSatoshis(0.1 * eacutil.SatoshiPerBitcoin)
