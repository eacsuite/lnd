...
import (
	...
	eacdnotify "github.com/eacsuite/lnd/chainntnfs/btcdnotify"
...
func loadTestCredits(miner *rpctest.Harness, w *lnwallet.LightningWallet,
	...
	satoshiPerOutput, err := eacutil.NewAmount(btcPerOutput)
	if err != nil {
	...
	expectedBalance += eacutil.Amount(int64(satoshiPerOutput) * int64(numOutputs))
	addrs := make([]eacutil.Address, 0, numOutputs)
...
func createTestWallet(tempTestDir string, miningNode *rpctest.Harness,
	...
	cfg := lnwallet.Config{
		...
		DefaultConstraints: channeldb.ChannelConstraints{
			DustLimit:        500,
			MaxPendingAmount: lnwire.NewMSatFromSatoshis(eacutil.SatoshiPerBitcoin) * 100,
			ChanReserve:      100,
			MinHTLC:          400,
...
func testDualFundingReservationWorkflow(miner *rpctest.Harness,
	...
	fundingAmount, err := eacutil.NewAmount(5)
	if err != nil {
...
func testFundingTransactionLockedOutputs(miner *rpctest.Harness,
	...
	fundingAmount, err := eacutil.NewAmount(8)
	if err != nil {
		t.Fatalf("unable to create amt: %v", err)
	}
	feePerKw, err := alice.Cfg.FeeEstimator.EstimateFeePerKW(1)
...
func testFundingCancellationNotEnoughFunds(miner *rpctest.Harness,
	...
	fundingAmount, err := eacutil.NewAmount(44)
	if err != nil {
		t.Fatalf("unable to create amt: %v", err)
	}
	req := &lnwallet.InitFundingReserveMsg{
...
func testReservationInitiatorBalanceBelowDustCancel(miner *rpctest.Harness,
	...
	const numBTC = 4
	fundingAmount, err := eacutil.NewAmount(numBTC)
	if err != nil {
		t.Fatalf("unable to create amt: %v", err)
	}

	feePerKw := chainfee.SatPerKWeight(
		numBTC * numBTC * eacutil.SatoshiPerBitcoin,
	)
	req := &lnwallet.InitFundingReserveMsg{
		ChainHash:        chainHash,
		NodeID:           bobPub,
...
func testSingleFunderReservationWorkflow(miner *rpctest.Harness,
	...
	// towards Bob's side.
	fundingAmt, err := eacutil.NewAmount(4)
	if err != nil {
		t.Fatalf("unable to create amt: %v", err)
	}
	pushAmt := lnwire.NewMSatFromSatoshis(eacutil.SatoshiPerBitcoin)
	feePerKw, err := alice.Cfg.FeeEstimator.EstimateFeePerKW(1)
	if err != nil {
		t.Fatalf("unable to query fee estimator: %v", err)
	}
...
func testListTransactionDetails(miner *rpctest.Harness,
	...
	const numTxns = 5
	const outputAmt = eacutil.SatoshiPerBitcoin
	...
	for _, txDetail := range txDetails {
		...
		if txOuts, ok := blockTxOuts[*txDetail.BlockHash][txDetail.Hash]; !ok {
			...
			var destinationAddresses []eacutil.Address
...
func scriptFromKey(pubkey *btcec.PublicKey) ([]byte, error) {
	...
	keyAddr, err := eacutil.NewAddressWitnessPubKeyHash(
...
func txFromOutput(tx *wire.MsgTx, signer input.Signer, fromPubKey,
	payToPubKey *btcec.PublicKey, txFee eacutil.Amount,
...
func newTx(t *testing.T, r *rpctest.Harness, pubKey *btcec.PublicKey,
	...
	newOutput := &wire.TxOut{
		Value:    eacutil.SatoshiPerBitcoin,
		PkScript: keyScript,
	}
	tx, err := alice.SendOutputs(
	...
	txFee := eacutil.Amount(0.001 * eacutil.SatoshiPerBitcoin)
...
func testPublishTransaction(r *rpctest.Harness,
	...
	// transactions.
	var (
		txFee         = eacutil.Amount(0.005 * eacutil.SatoshiPerBitcoin)
	...
	// backing eacd node will consider the tx being an orphan, and will
...
func testSignOutputUsingTweaks(r *rpctest.Harness,
	...
	for i := 0; i < 2; i++ {
		...
		pubkeyHash := eacutil.Hash160(tweakedKey.SerializeCompressed())
		keyAddr, err := eacutil.NewAddressWitnessPubKeyHash(pubkeyHash,
		...
		vm, err := txscript.NewEngine(keyScript,
			...
			nil, int64(eacutil.SatoshiPerBitcoin))
		if err != nil {
...
func testChangeOutputSpendConfirmation(r *rpctest.Harness,
	...
	output = &wire.TxOut{
		Value:    eacutil.SatoshiPerBitcent,
		PkScript: bobPkScript,
	}
	tx = sendCoins(t, r, alice, bob, output, txFeeRate)
	txHash = tx.TxHash()
...
func testSingleFunderExternalFundingTx(miner *rpctest.Harness,
	...
	// to Bob's side.
	chanAmt := 4 * eacutil.SatoshiPerBitcoin
	...
	fundingIntent, err := aliceChanFunder.ProvisionChannel(&chanfunding.Request{
		LocalAmt: eacutil.Amount(chanAmt),
		MinConfs: 1,
		FeeRate:  253,
		ChangeAddr: func() (eacutil.Address, error) {
	...
	aliceExternalFunder := chanfunding.NewCannedAssembler(
		thawHeight, *chanPoint, eacutil.Amount(chanAmt), &aliceFundingKey,
		bobFundingKey.PubKey, true,
	)
	bobShimIntent, err := chanfunding.NewCannedAssembler(
		thawHeight, *chanPoint, eacutil.Amount(chanAmt), &bobFundingKey,
...
func TestLightningWallet(t *testing.T) {
	...
	// Initialize the harness around a eacd node which will serve as our
	...
	chainNotifier, err := eacdnotify.New(
...
func runTests(t *testing.T, walletDriver *lnwallet.WalletDriver,
	...
	case "eacwallet":
		...
		case "neutrino":
			...
			// instance, and initialize a eacwallet driver for it.
		...
		case "bitcoind":
			...
			// Create a eacwallet bitcoind client for both Alice and
			// Bob.
		...
		if err != nil {
			t.Fatalf("unable to create eacwallet: %v", err)
		}
