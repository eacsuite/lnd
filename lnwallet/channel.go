...
func (c *commitment) toDiskCommit(ourCommit bool) *channeldb.ChannelCommitment {
	...
	commit := &channeldb.ChannelCommitment{
		...
		CommitFee:       c.fee,
		FeePerKw:        eacutil.Amount(c.feePerKw),
		CommitTx:        c.txn,
		CommitSig:       c.sig,
...
func (lc *LightningChannel) logUpdateToPayDesc(logUpdate *channeldb.LogUpdate,
	...
	remoteDustLimit eacutil.Amount) (*PaymentDescriptor, error) {
	...
	case *lnwire.UpdateFee:
		pd = &PaymentDescriptor{
			...
			Amount: lnwire.NewMSatFromSatoshis(
				eacutil.Amount(wireMsg.FeePerKw),
			),
			EntryType:                FeeUpdate,
			addCommitHeightRemote:    commitHeight,
			removeCommitHeightRemote: commitHeight,
...
func htlcIsDust(chanType channeldb.ChannelType,
	...
	htlcAmt, dustLimit eacutil.Amount) bool {
...
func NewUnilateralCloseSummary(chanState *channeldb.OpenChannel, signer input.Signer,
	...
	closeSummary := channeldb.ChannelCloseSummary{
		...
		SettledBalance:          eacutil.Amount(localBalance),
...
func (lc *LightningChannel) CreateCloseProposal(proposedFee eacutil.Amount,
...
func (lc *LightningChannel) CompleteCooperativeClose(
	...
	proposedFee eacutil.Amount) (*wire.MsgTx, eacutil.Amount, error) {
...
func (lc *LightningChannel) UpdateFee(feePerKw chainfee.SatPerKWeight) error {
	...
	pd := &PaymentDescriptor{
		LogIndex:  lc.localUpdateLog.logIndex,
		Amount:    lnwire.NewMSatFromSatoshis(eacutil.Amount(feePerKw)),
		EntryType: FeeUpdate,
	}
...
func CreateCooperativeCloseTx(fundingTxIn wire.TxIn,
	localDust, remoteDust, ourBalance, theirBalance eacutil.Amount,
...
// fee rate (fee-per-kw).
func (lc *LightningChannel) CalcFee(feeRate chainfee.SatPerKWeight) eacutil.Amount {
...
func (lc *LightningChannel) LocalChanReserve() eacutil.Amount {
