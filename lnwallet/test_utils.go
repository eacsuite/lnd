...
func CreateTestChannels(chanType channeldb.ChannelType) (
	*LightningChannel, *LightningChannel, func(), error) {
	...
	channelCapacity, err := eacutil.NewAmount(10)
	if err != nil {
...
func blockFromHex(blockHex string) (*eacutil.Block, error) {
	bytes, err := hex.DecodeString(blockHex)
	if err != nil {
		return nil, err
	}
	return eacutil.NewBlockFromBytes(bytes)
}

// txFromHex parses a full Bitcoin transaction from a hex encoded string.
func txFromHex(txHex string) (*eacutil.Tx, error) {
	bytes, err := hex.DecodeString(txHex)
	if err != nil {
		return nil, err
	}
	return eacutil.NewTxFromBytes(bytes)
}
...
func calcStaticFee(chanType channeldb.ChannelType, numHTLCs int) eacutil.Amount {
	const (
		htlcWeight = 172
		feePerKw   = eacutil.Amount(24/4) * 1000
	)
	return feePerKw *
		(eacutil.Amount(CommitWeight(chanType) +
