...
func TestCoinSelect(t *testing.T) {
	...
	const dust = eacutil.Amount(100)

	type testCase struct {
		name        string
		outputValue eacutil.Amount
		coins       []Coin

		expectedInput  []eacutil.Amount
		expectedChange eacutil.Amount
		expectErr      bool
	}
	...
	testCases := []testCase{
			...
			coins: []Coin{
				{
					TxOut: wire.TxOut{
						...
						Value:    1 * eacutil.SatoshiPerBitcoin,
					},
				},
			},
			outputValue: 0.5 * eacutil.SatoshiPerBitcoin,

			// The one and only input will be selected.
			expectedInput: []eacutil.Amount{
				1 * eacutil.SatoshiPerBitcoin,
			},
			// Change will be what's left minus the fee.
			expectedChange: 0.5*eacutil.SatoshiPerBitcoin - fundingFee(feeRate, 1, true),
		},
		{
			...
			outputValue: 1 * eacutil.SatoshiPerBitcoin,
			expectErr:   true,
		},
		{
			...
			outputValue: 1*eacutil.SatoshiPerBitcoin - fundingFee(feeRate, 1, true) - dust,
		...
		},
		{
			...
			outputValue: 1*eacutil.SatoshiPerBitcoin - fundingFee(feeRate, 1, true),
...
func TestCoinSelectSubtractFees(t *testing.T) {
	...
	const feeRate = chainfee.SatPerKWeight(100)
	const dustLimit = eacutil.Amount(1000)
	const dust = eacutil.Amount(100)

	type testCase struct {
		name       string
		spendValue eacutil.Amount
		coins      []Coin

		expectedInput      []eacutil.Amount
		expectedFundingAmt eacutil.Amount
		expectedChange     eacutil.Amount
		expectErr          bool
	}
	...
	testCases := []testCase{
		{
			...
			spendValue: 1 * eacutil.SatoshiPerBitcoin,

			// The one and only input will be selected.
			expectedInput: []eacutil.Amount{
				1 * eacutil.SatoshiPerBitcoin,
			},
			expectedFundingAmt: 1*eacutil.SatoshiPerBitcoin - fundingFee(feeRate, 1, false),
			expectedChange:     0,
		},
		{
		...
		},
		{
			...
			spendValue: 1*eacutil.SatoshiPerBitcoin - dust,
