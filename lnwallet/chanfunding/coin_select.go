...
type ErrInsufficientFunds struct {
	amountAvailable eacutil.Amount
	amountSelected  eacutil.Amount
}
...
// change+fees.
func selectInputs(amt eacutil.Amount, coins []Coin) (eacutil.Amount, []Coin, error) {
	satSelected := eacutil.Amount(0)
	for i, coin := range coins {
		satSelected += eacutil.Amount(coin.Value)
		if satSelected >= amt {
...
// function properly.
func CoinSelect(feeRate chainfee.SatPerKWeight, amt eacutil.Amount,
	coins []Coin) ([]Coin, eacutil.Amount, error) {
...
func CoinSelectSubtractFees(feeRate chainfee.SatPerKWeight, amt,
	dustLimit eacutil.Amount, coins []Coin) ([]Coin, eacutil.Amount,
