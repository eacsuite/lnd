...
// by the RPC interface of an active eacd node. This implementation will proxy
// any fee estimation requests to eacd's RPC interface.
...
// eacd node, and also a fall back fee rate. The fallback fee rate is used in
...
func (b *BtcdEstimator) Start() error {
	...
	info, err := b.eacdConn.GetInfo()
	if err != nil {
		return err
	}

	relayFee, err := eacutil.NewAmount(info.RelayFee)
	if err != nil {
		return err
	}
...
func (b *BtcdEstimator) fetchEstimate(confTarget uint32) (SatPerKWeight, error) {
	...
	btcPerKB, err := b.eacdConn.EstimateFee(int64(confTarget))
	if err != nil {
		return 0, err
	}
	...
	satPerKB, err := eacutil.NewAmount(btcPerKB)
	if err != nil {
		return 0, err
	}
...
func (b *BitcoindEstimator) fetchEstimate(confTarget uint32) (SatPerKWeight, error) {
	...
	// since it isn't supported by eacd but is available in bitcoind.
	...
	// returned in BTC.
	satPerKB, err := eacutil.NewAmount(feeEstimate.FeeRate)
	if err != nil {
		return 0, err
	}
