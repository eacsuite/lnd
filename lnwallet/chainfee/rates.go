...
type SatPerKVByte eacutil.Amount

// FeeForVSize calculates the fee resulting from this fee rate and the given
// vsize in vbytes.
func (s SatPerKVByte) FeeForVSize(vbytes int64) eacutil.Amount {
	return eacutil.Amount(s) * eacutil.Amount(vbytes) / 1000
}
...
type SatPerKWeight eacutil.Amount

// FeeForWeight calculates the fee resulting from this fee rate and the given
// weight in weight units (wu).
func (s SatPerKWeight) FeeForWeight(wu int64) eacutil.Amount {
	// The resulting fee is rounded down, as specified in BOLT#03.
	return eacutil.Amount(s) * eacutil.Amount(wu) / 1000
}
