...
func TestTxWeightEstimator(t *testing.T) {
	...
	p2pkhAddr, err := eacutil.NewAddressPubKeyHash(
	...
	p2wkhAddr, err := eacutil.NewAddressWitnessPubKeyHash(
		make([]byte, 20), netParams)
	if err != nil {
		t.Fatalf("Failed to generate address: %v", err)
	}
	...
	p2wshAddr, err := eacutil.NewAddressWitnessScriptHash(
	...
	p2shAddr, err := eacutil.NewAddressScriptHash([]byte{0}, netParams)
	if err != nil {
		t.Fatalf("Failed to generate address: %v", err)
	}
	...
	for i, test := range testCases {
		...
		expectedWeight := blockchain.GetTransactionWeight(eacutil.NewTx(tx))
