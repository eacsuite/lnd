...
func TestCommitSweepResolverNoDelay(t *testing.T) {
	...
	amt := eacutil.Amount(res.SelfOutputSignDesc.Output.Value)
...
func testCommitSweepResolverDelay(t *testing.T, sweepErr error) {
	...
	expectedReport := ContractReport{
		...
		Amount:       eacutil.Amount(amt),
		LimboBalance: eacutil.Amount(amt),
	}
	...
	expectedRecoveredBalance := eacutil.Amount(amt)
	if sweepErr != nil {
