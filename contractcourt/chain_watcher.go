...
type chainWatcherConfig struct {
	...
	// known to us.
	isOurAddr func(eacutil.Address) bool
...
// being dust.
func (c *chainWatcher) toSelfAmount(tx *wire.MsgTx) eacutil.Amount {
	var selfAmt eacutil.Amount
	for _, txOut := range tx.TxOut {
		...
		for _, addr := range addrs {
			if c.cfg.isOurAddr(addr) {
				selfAmt += eacutil.Amount(txOut.Value)
			}
		}
	}
...
func (c *chainWatcher) dispatchLocalForceClose(
	...
	for _, htlc := range forceClose.HtlcResolutions.OutgoingHTLCs {
		htlcValue := eacutil.Amount(htlc.SweepSignDesc.Output.Value)
