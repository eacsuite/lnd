...
func TestChannelLinkSingleHopPayment(t *testing.T) {
	...
	alice, bob, cleanUp, err := createTwoClusterChannels(
		eacutil.SatoshiPerBitcoin*3,
		eacutil.SatoshiPerBitcoin*5)
	if err != nil {
	...
	amount := lnwire.NewMSatFromSatoshis(eacutil.SatoshiPerBitcoin)
...
func TestExitNodeTimelockPayloadMismatch(t *testing.T) {
	...
	const amount = eacutil.SatoshiPerBitcoin
...
func TestLinkForwardFeePolicyMismatch(t *testing.T) {
	...
	amountNoFee := lnwire.NewMSatFromSatoshis(eacutil.SatoshiPerBitcoin)
...
func TestLinkForwardMaxHTLCPolicyMismatch(t *testing.T) {
	...
	channels, cleanUp, _, err := createClusterChannels(
		eacutil.SatoshiPerBitcoin*5, eacutil.SatoshiPerBitcoin*5,
	)
	if err != nil {
		t.Fatalf("unable to create channel: %v", err)
	}
...
func TestChannelLinkMultiHopInsufficientPayment(t *testing.T) {
	...
	// rejected.
	amount := lnwire.NewMSatFromSatoshis(4 * eacutil.SatoshiPerBitcoin)
	htlcAmt, totalTimelock, hops := generateHops(amount, testStartingHeight,
		n.firstBobChannelLink, n.carolChannelLink)
...
func newSingleLinkTestHarness(chanAmt, chanReserve eacutil.Amount) (
...
func TestChannelLinkBandwidthConsistency(t *testing.T) {
	...
	const chanAmt = eacutil.SatoshiPerBitcoin * 5
...
func TestChannelLinkTrimCircuitsPending(t *testing.T) {
	...
	const (
		chanAmt   = eacutil.SatoshiPerBitcoin * 5
		numHtlcs  = 4
...
func TestChannelLinkBandwidthChanReserve(t *testing.T) {
	...
	// channel reserve.
	const chanAmt = eacutil.SatoshiPerBitcoin * 5
	const chanReserve = eacutil.SatoshiPerBitcoin * 1
	...
	// a switch initiated payment.  The resulting bandwidth should
	// now be decremented to reflect the new HTLC.
	htlcAmt := lnwire.NewMSatFromSatoshis(3 * eacutil.SatoshiPerBitcoin)
	...
	const bobChanAmt = eacutil.SatoshiPerBitcoin * 1
	const bobChanReserve = eacutil.SatoshiPerBitcoin * 1.5
...
func TestChannelLinkUpdateCommitFee(t *testing.T) {
	...
	// for this test.
	const aliceInitialBalance = eacutil.SatoshiPerBitcoin * 3
	channels, cleanUp, _, err := createClusterChannels(
		aliceInitialBalance, eacutil.SatoshiPerBitcoin*5,
	)
	if err != nil {
		t.Fatalf("unable to create channel: %v", err)
	}
