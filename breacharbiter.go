...
justiceTxBroadcast:
	...
	case _, ok := <-confChan.Confirmed:
		...
		var totalFunds, revokedFunds eacutil.Amount
...
func (bo *breachedOutput) Amount() eacutil.Amount {
	return bo.amt
}
...
func (b *breachArbiter) sweepSpendableOutputsTxn(txWeight int64,
	...
	var totalAmt eacutil.Amount
	for _, input := range inputs {
		totalAmt += eacutil.Amount(input.SignDesc().Output.Value)
	}
...
func (bo *breachedOutput) Decode(r io.Reader) error {
	...
	bo.amt = eacutil.Amount(binary.BigEndian.Uint64(scratch[:8]))
