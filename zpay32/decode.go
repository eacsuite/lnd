...
// on-chain address.
func parseFallbackAddr(data []byte, net *chaincfg.Params) (eacutil.Address, error) {
	...
	case 0:
		...
		switch len(witness) {
		case 20:
			addr, err = eacutil.NewAddressWitnessPubKeyHash(witness, net)
		case 32:
			addr, err = eacutil.NewAddressWitnessScriptHash(witness, net)
		default:
	...
	case 17:
		...
		addr, err = eacutil.NewAddressPubKeyHash(pubKeyHash, net)
		if err != nil {
			return nil, err
		}
	case 18:
		...
		addr, err = eacutil.NewAddressScriptHashFromHash(scriptHash, net)
		if err != nil {
			return nil, err
		}
	default:
