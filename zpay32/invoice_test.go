...
var (
	...
	testAddrTestnet, _       = eacutil.DecodeAddress("mk2QpYatsKicvFVuTAQLBryyccRXMUaGHP", &chaincfg.TestNet4Params)
	testRustyAddr, _         = eacutil.DecodeAddress("1RustyRX2oai4EYYDpQGWvEL62BBGqN9T", &chaincfg.MainNetParams)
	testAddrMainnetP2SH, _   = eacutil.DecodeAddress("3EktnHQD7RiAE6uzMj2ZifT9YgRrkSgzQX", &chaincfg.MainNetParams)
	testAddrMainnetP2WPKH, _ = eacutil.DecodeAddress("bc1qw508d6qejxtdg4y5r3zarvary0c5xw7kv8f3t4", &chaincfg.MainNetParams)
	testAddrMainnetP2WSH, _  = eacutil.DecodeAddress("bc1qrp33g0q5c5txsp9arysrx4k6zdkfs4nce4xj0gdcccefvpysxf3qccfmv3", &chaincfg.MainNetParams)
	...
	eacTestNetParams chaincfg.Params
	eacMainNetParams chaincfg.Params
)

func init() {
	copy(testDescriptionHash[:], testDescriptionHashSlice[:])

	// Initialize earthcoin testnet and mainnet params by applying key fields
	// to copies of bitcoin params.
	// TODO(sangaman): create an interface for chaincfg.params
	eacTestNetParams = chaincfg.TestNet4Params
	eacTestNetParams.Net = wire.BitcoinNet(earthcoinCfg.TestNet4Params.Net)
	eacTestNetParams.Bech32HRPSegwit = earthcoinCfg.TestNet4Params.Bech32HRPSegwit
	eacMainNetParams = chaincfg.MainNetParams
	eacMainNetParams.Net = wire.BitcoinNet(earthcoinCfg.MainNetParams.Net)
	eacMainNetParams.Bech32HRPSegwit = earthcoinCfg.MainNetParams.Bech32HRPSegwit
}
...
func TestDecodeEncode(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// Decode a earthcoin testnet invoice
		...
		{
			// Decode a earthcoin mainnet invoice
...
func TestNewInvoice(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// Create a earthcoin testnet invoice
			newInvoice: func() (*Invoice, error) {
				return NewInvoice(&eacTestNetParams,
					testPaymentHash, time.Unix(1496314658, 0),
					Amount(testMillisat24BTC),
					DescriptionHash(testDescriptionHash),
					Destination(testPubKey))
		...
		{
			// Create a earthcoin mainnet invoice
			newInvoice: func() (*Invoice, error) {
				return NewInvoice(&eacMainNetParams,
					testPaymentHash, time.Unix(1496314658, 0),
					Amount(testMillisat24BTC),
					DescriptionHash(testDescriptionHash),
					Destination(testPubKey))
