...
* [Available Backend Operating Modes](#available-backend-operating-modes)
  * [eacd Options](#eacd-options)
  * [Neutrino Options](#neutrino-options)
  * [Bitcoind Options](#bitcoind-options)
  * [Using eacd](#using-eacd)
    * [Installing eacd](#installing-eacd)
    * [Starting eacd](#starting-eacd)
    * [Running lnd using the eacd backend](#running-lnd-using-the-eacd-backend)
  * [Using Neutrino](#using-neutrino)
  * [Using bitcoind or earthcoind](#using-bitcoind-or-earthcoind)
...
### Preliminaries
  In order to work with [`lnd`](https://github.com/eacsuite/lnd), the
...
```
git clone https://github.com/eacsuite/lnd
cd lnd
make install
```
...
release](https://github.com/eacsuite/lnd/releases). Assuming the name
...
commands:
```
cd $GOPATH/src/github.com/eacsuite/lnd
git pull
...
`eacd`, `neutrino`, `bitcoind`. All but neutrino (atm) can run on mainnet with
an out of the box `lnd` instance. We don't require `--txindex` when running
with `bitcoind` or `eacd` but activating the `txindex` will generally make
`lnd` run faster.
...
```
eacd:
      --eacd.dir=                                             The base directory that contains the node's data, logs, configuration file, etc. (default: /Users/roasbeef/Library/Application Support/eacd)
      --eacd.rpchost=                                         The daemon's rpc listening address. If a port is omitted, then the default port for the selected chain parameters will be used. (default: localhost)
      --eacd.rpcuser=                                         Username for RPC connections
      --eacd.rpcpass=                                         Password for RPC connections
      --eacd.rpccert=                                         File containing the daemon's certificate file (default: /Users/roasbeef/Library/Application Support/eacd/rpc.cert)
      --eacd.rawrpccert=                                      The raw bytes of the daemon's PEM-encoded certificate chain which will be used to authenticate the RPC connection.
```
...
```
earthcoind:
      --earthcoind.dir=                                         The base directory that contains the node's data, logs, configuration file, etc. (default: /Users/roasbeef/Library/Application Support/Bitcoin)
      --earthcoind.rpchost=                                     The daemon's rpc listening address. If a port is omitted, then the default port for the selected chain parameters will be used. (default: localhost)
      --earthcoind.rpcuser=                                     Username for RPC connections
      --earthcoind.rpcpass=                                     Password for RPC connections
      --earthcoind.zmqpubrawblock=                              The address listening for ZMQ connections to deliver raw block notifications
      --earthcoind.zmqpubrawtx=                                 The address listening for ZMQ connections to deliver raw transaction notifications
      --earthcoind.estimatemode=                                The fee estimate mode. Must be either "ECONOMICAL" or "CONSERVATIVE". (default: CONSERVATIVE)
```
...
To install eacd, run the following commands:
...
Alternatively, you can install [`eacd` directly from its
repo](https://github.com/eacsuite/eacd).

### Starting eacd

Running the following command will create `rpc.cert` and default `eacd.conf`.

```
eacd --testnet --rpcuser=REPLACEME --rpcpass=REPLACEME
```
If you want to use `lnd` on testnet, `eacd` needs to first fully sync the
...
While `eacd` is syncing you can check on its progress using eacd's `getinfo`
RPC command:
```
...
Additionally, you can monitor eacd's logs to track its syncing progress in real
time.

You can test your `eacd` node's connectivity using the `getpeerinfo` command:
```
btcctl --testnet --rpcuser=REPLACEME --rpcpass=REPLACEME getpeerinfo | more
```

### Running lnd using the eacd backend

If you are on testnet, run this command after `eacd` has finished syncing.
...
```
lnd --bitcoin.active --bitcoin.testnet --debuglevel=debug --eacd.rpcuser=kek --eacd.rpcpass=kek --externalip=X.X.X.X
```
...
in `--bitcoin.simnet` if needed), and also your own `eacd` node if available:
```
lnd --bitcoin.active --bitcoin.testnet --debuglevel=debug --bitcoin.node=neutrino --neutrino.connect=faucet.lightning.community
```


## Using bitcoind or earthcoind

The configuration for bitcoind and earthcoind are nearly identical, the
following steps can be mirrored with loss of generality to enable a earthcoind
backend.  Setup will be described in regards to `bitcoind`, but note that `lnd`
uses a distinct `earthcoin.node=earthcoind` argument and analogous
subconfigurations prefixed by `earthcoind`. Note that adding `--txindex` is
...
Github](https://github.com/eacsuite/lnd/issues/20).
...
start both `eacd` and `lnd` in the `simnet` mode. Simnet is similar to regtest
...
There are currently two primary ways to run `lnd`: one requires a local `eacd`
instance with the RPC service exposed, and the other uses a fully integrated
light client powered by [neutrino](https://github.com/eacsuite/neutrino).
...
Here's a sample `lnd.conf` for `eacd` to get you started:
```
[Application Options]
debuglevel=trace
...
Bitcoin chain. `lnd` also supports Earthcoin testnet4 (but not both BTC and EAC
at the same time), so when working with Earthcoin be sure to set to parameters
