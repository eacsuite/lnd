...
func createTestFundingManager(t *testing.T, privKey *btcec.PrivateKey,
	...
	fundingCfg := fundingConfig{
		...
		RequiredRemoteDelay: func(amt eacutil.Amount) uint16 {
			return 4
		},
...
func openChannel(t *testing.T, alice, bob *testNode, localFundingAmt,
	pushAmt eacutil.Amount, numConfs uint32,
...
func fundChannel(t *testing.T, alice, bob *testNode, localFundingAmt,
	pushAmt eacutil.Amount, subtractFees bool, numConfs uint32,
...
func assertChannelAnnouncements(t *testing.T, alice, bob *testNode,
	capacity eacutil.Amount, customMinHtlc []lnwire.MilliSatoshi,
	...
	for j, node := range nodes {
		...
		for _, msg := range announcements {
			...
			case *lnwire.ChannelUpdate:
				...
				// We might expect a custom MaxHeac value.
...
func TestFundingManagerNormalWorkflow(t *testing.T) {
	...
	localAmt := eacutil.Amount(500000)
...
func TestFundingManagerCustomChannelParameters(t *testing.T) {
	...
	localAmt := eacutil.Amount(5000000)
	pushAmt := eacutil.Amount(0)
	capacity := localAmt + pushAmt
	...
	// For maxHeac Alice should advertise the default MaxHtlc value of
...
func TestFundingManagerFundAll(t *testing.T) {
	...
	allCoins := []*lnwallet.Utxo{
		{
			...
			Value: eacutil.Amount(
				0.05 * eacutil.SatoshiPerBitcoin,
			),
		...
		},
		{
			AddressType: lnwallet.WitnessPubKey,
			Value: eacutil.Amount(
				0.06 * eacutil.SatoshiPerBitcoin,
			),
			PkScript: coinPkScript,
			OutPoint: wire.OutPoint{
				Hash:  chainhash.Hash{},
	...
	tests := []struct {
	...
	}{
		{
			...
			spendAmt: eacutil.Amount(
				0.11 * eacutil.SatoshiPerBitcoin,
			),
			change: false,
		},
		{
			...
			spendAmt: eacutil.Amount(
				0.10 * eacutil.SatoshiPerBitcoin,
			),
			change: true,
		},
	}
...
func TestWumboChannelConfig(t *testing.T) {
	...
	// should be rejected.
	initReq.localFundingAmt = eacutil.SatoshiPerBitcoin
