...
import (
	...
	_ "github.com/eacsuite/eacwallet/walletdb/bdb" // Required in order to create the default database.
)
...
func TestKeyRingDerivation(t *testing.T) {
	...
	keyRingImplementations := []keyRingConstructor{
		func() (string, func(), KeyRing, error) {
			...
			return "eacwallet", cleanUp, keyRing, nil
		},
		func() (string, func(), KeyRing, error) {
			cleanUp, wallet, err := createTestBtcWallet(
				CoinTypeEarthcoin,
			...
			keyRing := NewBtcWalletKeyRing(wallet, CoinTypeEarthcoin)
