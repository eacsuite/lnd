...
import (
	...
	"github.com/eacsuite/lnd/build"
	"github.com/eacsuite/lnd/chanacceptor"
	"github.com/eacsuite/lnd/chanbackup"
	"github.com/eacsuite/lnd/chanfitness"
	"github.com/eacsuite/lnd/channeldb"
	"github.com/eacsuite/lnd/channeldb/kvdb"
	"github.com/eacsuite/lnd/channelnotifier"
	"github.com/eacsuite/lnd/contractcourt"
	"github.com/eacsuite/lnd/discovery"
	"github.com/eacsuite/lnd/feature"
	"github.com/eacsuite/lnd/htlcswitch"
	"github.com/eacsuite/lnd/input"
	"github.com/eacsuite/lnd/invoices"
	"github.com/eacsuite/lnd/keychain"
	"github.com/eacsuite/lnd/labels"
	...
	"github.com/eacsuite/lnd/lnwallet/chanfunding"
	"github.com/eacsuite/lnd/lnwire"
	"github.com/eacsuite/lnd/macaroons"
	"github.com/eacsuite/lnd/monitoring"
	"github.com/eacsuite/lnd/peer"
	"github.com/eacsuite/lnd/peernotifier"
	"github.com/eacsuite/lnd/record"
	"github.com/eacsuite/lnd/routing"
	"github.com/eacsuite/lnd/routing/route"
	"github.com/eacsuite/lnd/signal"
	"github.com/eacsuite/lnd/sweep"
	"github.com/eacsuite/lnd/watchtower"
	"github.com/eacsuite/lnd/zpay32"
	...
	"github.com/eacsuite/eacutil/psbt"
...
func newRPCServer(cfg *Config, s *server, macService *macaroons.Service,
	...
	routerBackend := &routerrpc.RouterBackend{
		...
		FetchChannelCapacity: func(chanID uint64) (eacutil.Amount,
			error) {
...
func addrPairsToOutputs(addrPairs map[string]int64) ([]*wire.TxOut, error) {
	...
	for addr, amt := range addrPairs {
		addr, err := eacutil.DecodeAddress(addr, activeNetParams.Params)
		if err != nil {
			return nil, err
		}
...
func (r *rpcServer) SendCoins(ctx context.Context,
	...
	rpcsLog.Infof("[sendcoins] addr=%v, amt=%v, sat/kw=%v, sweep_all=%v",
		in.Addr, eacutil.Amount(in.Amount), int64(feePerKw),
		in.SendAll)

	// Decode the address receiving the coins, we need to check whether the
	// address is valid for this network.
	targetAddr, err := eacutil.DecodeAddress(in.Addr, activeNetParams.Params)
	if err != nil {
		return nil, err
	}
...
func newFundingShimAssembler(chanPointShim *lnrpc.ChanPointShim, initiator bool,
	...
	return chanfunding.NewCannedAssembler(
		...
		eacutil.Amount(chanPointShim.Amt), &localKeyDesc,
		remoteKey, initiator,
	), nil
}
...
func newPsbtAssembler(req *lnrpc.OpenChannelRequest, normalizedMinConfs int32,
	...
	return chanfunding.NewPsbtAssembler(
		eacutil.Amount(req.LocalFundingAmount), packet, netParams,
		!psbtShim.NoPublish,
	), nil
}
...
func (r *rpcServer) parseOpenChannelReq(in *lnrpc.OpenChannelRequest,
	...
	localFundingAmt := eacutil.Amount(in.LocalFundingAmount)
	remoteInitialBalance := eacutil.Amount(in.PushSat)
...
func parseUpfrontShutdownAddress(address string) (lnwire.DeliveryAddress, error) {
	...
	addr, err := eacutil.DecodeAddress(
...
func (r *rpcServer) ChannelBalance(ctx context.Context,
	...
	var pendingOpenBalance eacutil.Amount
...
func (r *rpcServer) PendingChannels(ctx context.Context,
	...
	for i, pendingChan := range pendingOpenChannels {
		...
		utx := eacutil.NewTx(localCommitment.CommitTx)
...
func createRPCOpenChannel(r *rpcServer, graph *channeldb.ChannelGraph,
	...
	utx := eacutil.NewTx(localCommit.CommitTx)
	commitBaseWeight := blockchain.GetTransactionWeight(utx)
	commitWeight := commitBaseWeight + input.WitnessCommitmentTxWeight
	...
	var sumOutputs eacutil.Amount
	for _, txOut := range localCommit.CommitTx.TxOut {
		sumOutputs += eacutil.Amount(txOut.Value)
	}
...
func (r *rpcServer) AddInvoice(ctx context.Context,
	...
	if r.cfg.registeredChains.PrimaryChain() == earthcoinChain {
		defaultDelta = r.cfg.Earthcoin.TimeLockDelta
	}
...
func (r *rpcServer) GetNetworkInfo(ctx context.Context,
	...
	var (
		...
		numChannels          uint32
		maxChanOut           uint32
		totalNetworkCapacity eacutil.Amount
		minChannelSize       eacutil.Amount = math.MaxInt64
		maxChannelSize       eacutil.Amount
		medianChanSize       eacutil.Amount
	)
...
func (r *rpcServer) FundingStateStep(ctx context.Context,
	...
	case in.GetShimRegister() != nil &&
		...
		req := &chanfunding.Request{
			RemoteAmt: eacutil.Amount(rpcShimIntent.Amt),
		}
