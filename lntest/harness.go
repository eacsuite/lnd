...
func (n *NetworkHarness) SetUp(lndArgs []string) error {
	...
	for _, client := range clients {
		for i := 0; i < 10; i++ {
			...
			addr, err := eacutil.DecodeAddress(resp.Address, n.netParams)
			if err != nil {
				return err
			}
	...
	expectedBalance := int64(eacutil.SatoshiPerBitcoin * 10)
...
func (n *NetworkHarness) OpenPendingChannel(ctx context.Context,
	srcNode, destNode *HarnessNode, amt eacutil.Amount,
	pushAmt eacutil.Amount) (*lnrpc.PendingUpdate, error) {
...
func (n *NetworkHarness) SendCoins(ctx context.Context, amt eacutil.Amount,
...
func (n *NetworkHarness) SendCoinsUnconfirmed(ctx context.Context,
	amt eacutil.Amount, target *HarnessNode) error {
...
func (n *NetworkHarness) sendCoins(ctx context.Context, amt eacutil.Amount,
	...
	// and exit.
	if !confirmed {
		expectedBalance := eacutil.Amount(initialBalance.UnconfirmedBalance) + amt
	...
	expectedBalance := eacutil.Amount(initialBalance.ConfirmedBalance) + amt
