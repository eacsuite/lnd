...
func (hn *HarnessNode) WaitForBalance(expectedBalance eacutil.Amount, confirmed bool) error {
	ctx := context.Background()
	req := &lnrpc.WalletBalanceRequest{}

	var lastBalance eacutil.Amount
	doesBalanceMatch := func() bool {
		...
		if confirmed {
			lastBalance = eacutil.Amount(balance.ConfirmedBalance)
			return eacutil.Amount(balance.ConfirmedBalance) == expectedBalance
		}

		lastBalance = eacutil.Amount(balance.UnconfirmedBalance)
		return eacutil.Amount(balance.UnconfirmedBalance) == expectedBalance
	}
