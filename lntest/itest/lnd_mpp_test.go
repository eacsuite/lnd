...
func testSendToRouteMultiPath(net *lntest.NetworkHarness, t *harnessTest) {
	...
	const (
		paymentAmt = eacutil.Amount(300000)
	...
	buildRoute := func(amt eacutil.Amount, hops []*lntest.HarnessNode) (
		*lnrpc.Route, error) {
...
func (c *mppTestContext) openChannel(from, to *lntest.HarnessNode, chanSize eacutil.Amount) {
	ctxb := context.Background()

	ctxt, _ := context.WithTimeout(ctxb, defaultTimeout)
	err := c.net.SendCoins(ctxt, eacutil.SatoshiPerBitcoin, from)
	if err != nil {
