...
import (
	...
	"github.com/eacsuite/lnd"
	"github.com/eacsuite/lnd/channeldb"
	"github.com/eacsuite/lnd/input"
	"github.com/eacsuite/lnd/lncfg"
	"github.com/eacsuite/lnd/lnrpc"
	"github.com/eacsuite/lnd/lnrpc/invoicesrpc"
	"github.com/eacsuite/lnd/lnrpc/routerrpc"
	"github.com/eacsuite/lnd/lnrpc/signrpc"
	"github.com/eacsuite/lnd/lnrpc/walletrpc"
	"github.com/eacsuite/lnd/lnrpc/watchtowerrpc"
	"github.com/eacsuite/lnd/lnrpc/wtclientrpc"
	"github.com/eacsuite/lnd/lntest"
	"github.com/eacsuite/lnd/lntest/wait"
	"github.com/eacsuite/lnd/lntypes"
	"github.com/eacsuite/lnd/lnwallet"
	"github.com/eacsuite/lnd/lnwallet/chainfee"
	"github.com/eacsuite/lnd/lnwire"
	"github.com/eacsuite/lnd/routing"
...
func createPayReqs(node *lntest.HarnessNode, paymentAmt eacutil.Amount,
...
func testOnchainFundRecovery(net *lntest.NetworkHarness, t *harnessTest) {
	...
	skipAndSend := func(nskip int) func(*lntest.HarnessNode) {
		return func(node *lntest.HarnessNode) {
			...
			ctxt, _ := context.WithTimeout(ctxb, defaultTimeout)
			err = net.SendCoins(
				ctxt, eacutil.SatoshiPerBitcoin, node,
			)
			if err != nil {
	...
	// path.
	restoreCheckBalance(2*eacutil.SatoshiPerBitcoin, 2, 1, skipAndSend(9))

	// Check that using a recovery window of 9 does not find the two most
	// recent txns.
	restoreCheckBalance(2*eacutil.SatoshiPerBitcoin, 2, 9, nil)
	...
	restoreCheckBalance(4*eacutil.SatoshiPerBitcoin, 4, 10, skipAndSend(19))

	// Check that recovering with a recovery window of 19 fails to find the
	// most recent transactions.
	restoreCheckBalance(4*eacutil.SatoshiPerBitcoin, 4, 19, nil)
	...
	const minerAmt = 5 * eacutil.SatoshiPerBitcoin
	const finalBalance = 6 * eacutil.SatoshiPerBitcoin
...
func basicChannelFundingTest(t *harnessTest, net *lntest.NetworkHarness,
	...
	aliceBalance := eacutil.Amount(aliceBal.Balance)
	...
	bobBalance := eacutil.Amount(bobBal.Balance)
...
test:
	...
	for _, carolCommitType := range allTypes {
		for _, daveCommitType := range allTypes {
			...
			ctxt, _ := context.WithTimeout(ctxb, defaultTimeout)
			err = net.SendCoins(ctxt, eacutil.SatoshiPerBitcoin, carol)
			if err != nil {
...
func testPaymentFollowingChannelOpen(net *lntest.NetworkHarness, t *harnessTest) {
	...
	const paymentAmt = eacutil.Amount(100)
	channelCapacity := eacutil.Amount(paymentAmt * 1000)
...
func calculateMaxHtlc(chanCap eacutil.Amount) uint64 {
...
func testDisconnectingTargetPeer(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoins(ctxt, eacutil.SatoshiPerBitcoin, alice)
	if err != nil {
...
func testChannelBalance(net *lntest.NetworkHarness, t *harnessTest) {
	...
	checkChannelBalance := func(node lnrpc.LightningClient,
		...
		balance := eacutil.Amount(response.Balance)
		if balance != amount {
...
func testChannelUnsettledBalance(net *lntest.NetworkHarness, t *harnessTest) {
	const chanAmt = eacutil.Amount(1000000)
	ctxb := context.Background()
...
func testChannelForceClosure(net *lntest.NetworkHarness, t *harnessTest) {
	...
	for _, channelType := range commitTypes {
		...
		success := t.t.Run(testName, func(t *testing.T) {
			...
			err = net.SendCoins(
				ctxt, eacutil.SatoshiPerBitcoin, alice,
			)
			if err != nil {
			...
			// anchor.
			err = net.SendCoins(
				ctxt, eacutil.SatoshiPerBitcoin, carol,
			)
			if err != nil {
				t.Fatalf("unable to send coins to Alice: %v",
					err)
...
func channelForceClosureTest(net *lntest.NetworkHarness, t *harnessTest,
	...
	const (
		chanAmt     = eacutil.Amount(10e6)
		pushAmt     = eacutil.Amount(5e6)
		paymentAmt  = 100000
		numInvoices = 6
	)
	...
	carolExpectedBalance := eacutil.Amount(carolStartingBalance) + pushAmt
	if eacutil.Amount(carolBalResp.ConfirmedBalance) < carolExpectedBalance {
		t.Fatalf("carol's balance is incorrect: expected %v got %v",
...
func assertAmountSent(amt eacutil.Amount, sndr, rcvr *lntest.HarnessNode) func() error {
	return func() error {
...
func testSphinxReplayPersistence(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoins(ctxt, eacutil.SatoshiPerBitcoin, fred)
	if err != nil {
...
func testSingleHopSendToRouteCase(net *lntest.NetworkHarness, t *harnessTest,
	...
	const chanAmt = eacutil.Amount(100000)
...
func testMultiHopSendToRoute(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoins(ctxt, eacutil.SatoshiPerBitcoin, net.Bob)
	if err != nil {
...
func testSendToRouteErrorPropagation(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoins(ctxt, eacutil.SatoshiPerBitcoin, charlie)
	if err != nil {
...
func testPrivateChannels(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoins(ctxt, eacutil.SatoshiPerBitcoin, dave)
	if err != nil {
...
func testInvoiceSubscriptions(net *lntest.NetworkHarness, t *harnessTest) {
	...
	const chanAmt = eacutil.Amount(500000)
...
func testMaxPendingChannels(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	carolBalance := eacutil.Amount(maxPendingChannels) * amount
...
func testRevokedCloseRetributionAltruistWatchtower(net *lntest.NetworkHarness,
	...
	// Before we make a channel, we'll load up Dave with some coins sent
	// directly from the miner.
	err = net.SendCoins(ctxb, eacutil.SatoshiPerBitcoin, dave)
	if err != nil {
		t.Fatalf("unable to send coins to dave: %v", err)
	}
...
func testRejectHTLC(net *lntest.NetworkHarness, t *harnessTest) {
	...
	// Send coins to Carol.
	err = net.SendCoins(ctxb, eacutil.SatoshiPerBitcoin, carol)
	if err != nil {
		t.Fatalf("unable to send coins to carol: %v", err)
	}

	// Send coins to Alice.
	err = net.SendCoins(ctxb, eacutil.SatoshiPerBitcent, net.Alice)
	if err != nil {
		t.Fatalf("unable to send coins to alice: %v", err)
	}
			...
			for _, chanUpdate := range graphUpdate.ChannelUpdates {
				...
				if chanUpdate.Capacity != int64(chanAmt) {
					t.Fatalf("channel capacities mismatch:"+
						...
						eacutil.Amount(chanUpdate.Capacity))
				}
				numChannelUpds++
			}
...
func testAsyncPayments(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ := context.WithTimeout(ctxb, channelOpenTimeout)
	channelCapacity := eacutil.Amount(paymentAmt * 2000)
	chanPoint := openChannelAndAssert(
		ctxt, t, net, net.Alice, net.Bob,
		lntest.OpenChannelParams{
			Amt: channelCapacity,
...
func testSwitchCircuitPersistence(net *lntest.NetworkHarness, t *harnessTest) {
	...
	const chanAmt = eacutil.Amount(1000000)
	const pushAmt = eacutil.Amount(900000)
	var networkChans []*lnrpc.ChannelPoint
...
func testSendUpdateDisableChannel(net *lntest.NetworkHarness, t *harnessTest) {
	...
	// Give Eve some coins.
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoins(ctxt, eacutil.SatoshiPerBitcoin, eve)
	if err != nil {
...
func testSweepAllCoins(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ := context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoins(ctxt, eacutil.SatoshiPerBitcoin, ainz)
	if err != nil {
		t.Fatalf("unable to send coins to eve: %v", err)
	}
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoinsNP2WKH(ctxt, eacutil.SatoshiPerBitcoin, ainz)
	if err != nil {
		t.Fatalf("unable to send coins to eve: %v", err)
	}
...
func testHoldInvoicePersistence(net *lntest.NetworkHarness, t *harnessTest) {
	...
	const (
		chanAmt     = eacutil.Amount(1000000)
		numPayments = 10
	)
...
func TestLightningNetworkDaemon(t *testing.T) {
	...
	// Create an instance of the eacd's rpctest.Harness that will act as
	...
	handlers := &rpcclient.NotificationHandlers{
		OnTxAccepted: func(hash *chainhash.Hash, amt eacutil.Amount) {
	...
	defer func() {
		...
		logFile := fmt.Sprintf(
			"%s/%s/eacd.log", minerLogDir, harnessNetParams.Name,
		)
		err := lntest.CopyFile("./output_eacd_miner.log", logFile)
		if err != nil {
	...
	// With the eacd harness created, we can now complete the
