// Copyright (c) 2013-2017 The eacsuite developers
...
import (
	...
	"github.com/eacsuite/lnd/htlcswitch/hodl"
...
var (
	...
	DefaultLndDir = eacutil.AppDataDir("lnd", false)
...
func DefaultConfig() Config {
	return Config{
		...
		Earthcoin: &lncfg.Chain{
			MinHTLCIn:     defaultEarthcoinMinHTLCInMSat,
			MinHTLCOut:    defaultEarthcoinMinHTLCOutMSat,
			BaseFee:       defaultEarthcoinBaseFeeMSat,
			FeeRate:       defaultEarthcoinFeeRate,
...
func ValidateConfig(cfg Config, usageMessage string) (*Config, error) {
	...
	case cfg.Earthcoin.Active && cfg.Bitcoin.Active:
		str := "%s: Currently both Bitcoin and Earthcoin cannot be " +
			"active together"
		return nil, fmt.Errorf(str, funcName)

	// Either Bitcoin must be active, or Earthcoin must be active.
	// Otherwise, we don't know which chain we're on.
	case !cfg.Bitcoin.Active && !cfg.Earthcoin.Active:
		return nil, fmt.Errorf("%s: either bitcoin.active or "+
			"earthcoin.active must be set to 1 (true)", funcName)
	...
	case cfg.Earthcoin.Active:
		...
		// while we're at it.
		numNets := 0
		if cfg.Earthcoin.MainNet {
			numNets++
			activeNetParams = earthcoinMainNetParams
		}
		if cfg.Earthcoin.TestNet3 {
			numNets++
			activeNetParams = earthcoinTestNetParams
		}
		if cfg.Earthcoin.RegTest {
			numNets++
			activeNetParams = earthcoinRegTestNetParams
		}
		if cfg.Earthcoin.SimNet {
			numNets++
			activeNetParams = earthcoinSimNetParams
		}
		...
		if numNets == 0 {
			str := "%s: either --earthcoin.mainnet, or " +
				"earthcoin.testnet must be specified"
			err := fmt.Errorf(str, funcName)
			return nil, err
		}

		if cfg.Earthcoin.TimeLockDelta < minTimeLockDelta {
		...
		case "eacd":
			...
			if err != nil {
				err := fmt.Errorf("unable to load RPC "+
					"credentials for eacd: %v", err)
				return nil, err
			}
		case "earthcoind":
			if cfg.Earthcoin.SimNet {
				return nil, fmt.Errorf("%s: earthcoind does not "+
					"support simnet", funcName)
			}
			err := parseRPCParams(cfg.Earthcoin, cfg.EarthcoindMode,
				earthcoinChain, funcName)
			if err != nil {
				err := fmt.Errorf("unable to load RPC "+
					"credentials for earthcoind: %v", err)
				return nil, err
			}
		case "neutrino":
		...
		default:
			str := "%s: only eacd, earthcoind, and neutrino mode " +
				"supported for earthcoin at this time"
			return nil, fmt.Errorf(str, funcName)
		}

		cfg.Earthcoin.ChainDir = filepath.Join(cfg.DataDir,
			defaultChainSubDirname,
			earthcoinChain.String())
...
func parseRPCParams(cConfig *lncfg.Chain, nodeConfig interface{}, net chainCode,
	...
	// If we're in simnet mode, then the running eacd instance won't read
	// the RPC credentials from the configuration. So if lnd wasn't
	// specified the parameters, then we won't be able to start.
	if cConfig.SimNet {
		str := "%v: rpcuser and rpcpass must be set to your eacd " +
...
// eacd instance. The passed path is expected to be the location of eacd's
// application data directory on the target system.
func extractBtcdRPCParams(btcdConfigPath string) (string, string, error) {
	// First, we'll open up the eacd configuration file found at the target
	// destination.
