...
import (
	...
	"github.com/eacsuite/lnd/chainntnfs"
	"github.com/eacsuite/lnd/chainntnfs/bitcoindnotify"
	"github.com/eacsuite/lnd/chainntnfs/btcdnotify"
	"github.com/eacsuite/lnd/chainntnfs/neutrinonotify"
	"github.com/eacsuite/lnd/channeldb"
	"github.com/eacsuite/lnd/htlcswitch"
	...
	"github.com/eacsuite/lnd/lnwallet/btcwallet"
	"github.com/eacsuite/lnd/lnwallet/chainfee"
	"github.com/eacsuite/lnd/lnwire"
	"github.com/eacsuite/lnd/routing/chainview"
...
const (
	...
	defaultEarthcoinMinHTLCInMSat  = lnwire.MilliSatoshi(1)
	defaultEarthcoinMinHTLCOutMSat = lnwire.MilliSatoshi(1000)
	defaultEarthcoinBaseFeeMSat    = lnwire.MilliSatoshi(1000)
	defaultEarthcoinFeeRate        = lnwire.MilliSatoshi(1)
	defaultEarthcoinTimeLockDelta  = 576
	defaultEarthcoinDustLimit      = eacutil.Amount(54600)
	...
	// defaultEarthcoinStaticFeePerKW is the fee rate of 200 sat/vbyte
	// expressed in sat/kw.
...
const (
	...
	// earthcoinChain is Earthcoin's testnet chain.
	earthcoinChain
)
...
// branches of chainControl instances exist: one backed by a running eacd
...
func newChainControlFromConfig(cfg *Config, localDB, remoteDB *channeldb.DB,
	...
	case earthcoinChain:
		cc.routingPolicy = htlcswitch.ForwardingPolicy{
			MinHTLCOut:    cfg.Earthcoin.MinHTLCOut,
			BaseFee:       cfg.Earthcoin.BaseFee,
			FeeRate:       cfg.Earthcoin.FeeRate,
			TimeLockDelta: cfg.Earthcoin.TimeLockDelta,
		}
		cc.minHtlcIn = cfg.Earthcoin.MinHTLCIn
		cc.feeEstimator = chainfee.NewStaticEstimator(
			defaultEarthcoinStaticFeePerKW, 0,
		)
	default:
	...
	case "bitcoind", "earthcoind":
		var bitcoindMode *lncfg.Bitcoind
		switch {
		case cfg.Bitcoin.Active:
			bitcoindMode = cfg.BitcoindMode
		case cfg.Earthcoin.Active:
			bitcoindMode = cfg.EarthcoindMode
		}
		// Otherwise, we'll be speaking directly via RPC and ZMQ to a
		// bitcoind node. If the specified host for the eacd/eacd RPC
		...
		if strings.Contains(bitcoindMode.RPCHost, ":") {
			...
			// eacd, which picks a different port so that btcwallet
			...
			if (cfg.Bitcoin.Active && cfg.Bitcoin.RegTest) ||
				(cfg.Earthcoin.Active && cfg.Earthcoin.RegTest) {
				conn, err := net.Dial("tcp", bitcoindHost)
				if err != nil || conn == nil {
					if cfg.Bitcoin.Active && cfg.Bitcoin.RegTest {
						rpcPort = 18443
					} else if cfg.Earthcoin.Active && cfg.Earthcoin.RegTest {
						rpcPort = 19443
					}
					bitcoindHost = fmt.Sprintf("%v:%d",
		...
		if cfg.Bitcoin.Active && !cfg.Bitcoin.RegTest {
			...
			// coded value.
			fallBackFeeRate := chainfee.SatPerKVByte(10 * 1000)
			...
			ltndLog.Infof("Initializing earthcoind backed fee estimator in "+
				"%s mode", bitcoindMode.EstimateMode)

			// Finally, we'll re-initialize the fee estimator, as
			// if we're using earthcoind as a backend, then we can
			// use live fee estimates, rather than a statically
			// coded value.
			fallBackFeeRate := chainfee.SatPerKVByte(10 * 1000)
			cc.feeEstimator, err = chainfee.NewBitcoindEstimator(
	...
	case "btcd", "eacd":
		...
		//
		// So first we'll load eacd/eacd's TLS cert for the RPC
		...
		// If the specified host for the eacd/eacd RPC server already
		...
		cc.chainNotifier, err = eacdnotify.New(
		...
		// Create a special websockets rpc client for eacd which will be used
		...
		if !cfg.Bitcoin.SimNet && !cfg.Earthcoin.SimNet &&
			!cfg.Bitcoin.RegTest && !cfg.Earthcoin.RegTest {

			ltndLog.Infof("Initializing eacd backed fee estimator")

			// Finally, we'll re-initialize the fee estimator, as
			// if we're using eacd as a backend, then we can use
...
var (
	...
	// earthcoinTestnetGenesis is the genesis hash of Earthcoin's testnet4
	// chain.
	earthcoinTestnetGenesis = chainhash.Hash([chainhash.HashSize]byte{
	...
	// earthcoinMainnetGenesis is the genesis hash of Earthcoin's main chain.
	earthcoinMainnetGenesis = chainhash.Hash([chainhash.HashSize]byte{
	...
	chainMap = map[chainhash.Hash]chainCode{
		...
		earthcoinTestnetGenesis: earthcoinChain,

		bitcoinMainnetGenesis:  bitcoinChain,
		earthcoinMainnetGenesis: earthcoinChain,
	}
	...
	chainDNSSeeds = map[chainhash.Hash][][2]string{
		...
		earthcoinMainnetGenesis: {
			{
				"eac.nodes.lightning.directory",
				"soa.nodes.lightning.directory",
			},
		},
	}
