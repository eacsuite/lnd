...
import (
	...
	_ "github.com/eacsuite/eacwallet/walletdb/bdb" // Required to register the boltdb walletdb implementation.
...
var (
	...
	addrPk, _       = eacutil.NewAddressPubKey(pubKey.SerializeCompressed(),
		netParams)
...
func testFilterSingleBlock(node *rpctest.Harness, chainView FilteredChainView,
	...
	txns := []*eacutil.Tx{eacutil.NewTx(spendingTx1), eacutil.NewTx(spendingTx2)}
...
func testFilterBlockDisconnected(node *rpctest.Harness,
	...
	if err = reorgView.Start(); err != nil {
		t.Fatalf("unable to start eacd chain view: %v", err)
	}
