...
// interface which is backed by an active websockets connection to eacd.
...
// RPC credentials for an active eacd instance.
func NewBtcdFilteredChainView(config rpcclient.ConnConfig) (*BtcdFilteredChainView, error) {
	...
	// Disable connecting to eacd within the rpcclient.New method. We
...
func (b *BtcdFilteredChainView) Start() error {
	...
	// Connect to eacd, and register for notifications on connected, and
	// disconnected blocks.
	if err := b.eacdConn.Connect(20); err != nil {
		return err
	}
	if err := b.eacdConn.NotifyBlocks(); err != nil {
		return err
	}

	_, bestHeight, err := b.eacdConn.GetBestBlock()
	if err != nil {
		return err
	}
...
func (b *BtcdFilteredChainView) Stop() error {
	...
	// Shutdown the rpc client, this gracefully disconnects from eacd, and
...
func (b *BtcdFilteredChainView) onFilteredBlockConnected(height int32,
	header *wire.BlockHeader, txns []*eacutil.Tx) {
...
func (b *BtcdFilteredChainView) chainFilterer() {
	...
	for {
		...
		case update := <-b.filterUpdates:
			...
			// Apply the new TX filter to eacd, which will cause
			// all following notifications from and calls to it
			// return blocks filtered with the new filter.
			b.eacdConn.LoadTxFilter(false, []eacutil.Address{},
				update.newUtxos)
			...
			// with eacd applying the newly loaded filter to each
			// block.
			for i := update.updateHeight + 1; i < bestHeight+1; i++ {
				blockHash, err := b.eacdConn.GetBlockHash(int64(i))
				if err != nil {
				...
				// have gone missing.
				rescanned, err := b.eacdConn.RescanBlocks(
		...
		case req := <-b.filterBlockReqs:
			...
			block, err := b.eacdConn.GetBlock(req.blockHash)
			if err != nil {
				req.err <- err
				req.resp <- nil
				continue
			}
			header, err := b.eacdConn.GetBlockHeaderVerbose(req.blockHash)
			if err != nil {
				req.err <- err
				req.resp <- nil
				continue
