...
func parseTestGraph(path string) (*testGraphInstance, error) {
	...
	for _, edge := range g.Edges {
		...
		edgeInfo := channeldb.ChannelEdgeInfo{
			...
			Capacity:     eacutil.Amount(edge.Capacity),
		}
...
func symmetricTestChannel(alias1, alias2 string, capacity eacutil.Amount,
...
func asymmetricTestChannel(alias1, alias2 string, capacity eacutil.Amount,
...
func TestPathInsufficientCapacity(t *testing.T) {
	...
	payAmt := lnwire.NewMSatFromSatoshis(eacutil.SatoshiPerBitcoin)
	_, err = dbFindPath(
		graph.graph, nil, nil,
		noRestrictions, testPathFindingConfig,
