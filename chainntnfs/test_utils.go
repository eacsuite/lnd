...
func randPubKeyHashScript() ([]byte, *btcec.PrivateKey, error) {
	...
	addrScript, err := eacutil.NewAddressPubKeyHash(pubKeyHash, NetParams)
	if err != nil {
		return nil, nil, err
	}
...
// NewMiner spawns testing harness backed by a eacd node that can serve as a
// miner.
...
func NewNeutrinoBackend(t *testing.T, minerAddr string) (*neutrino.ChainService, func()) {
	...
	// Create an instance of neutrino connected to the running eacd
	// instance.
	...
	// generated by the eacd instance.
	spvNode.Start()
