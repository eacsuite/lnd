...
// txUpdate encapsulates a transaction related notification sent from eacd to
...
// BtcdNotifier implements the ChainNotifier interface using eacd's websockets
...
// New returns a new BtcdNotifier instance. This function assumes the eacd node
...
// Start connects to the running eacd node over websockets, registers for block
...
func (b *BtcdNotifier) onRedeemingTx(tx *eacutil.Tx, details *btcjson.BlockDetails) {
...
func (b *BtcdNotifier) handleBlockConnected(epoch chainntnfs.BlockEpoch) error {
	...
	newBlock := &filteredBlock{
		...
		txns:    eacutil.NewBlock(rawBlock).Transactions(),
		connect: true,
	}
